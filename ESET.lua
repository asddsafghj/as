...
local notify = lgi.require("Notifyl")
local chats = {}
local minute = 60
local hour = 3600
local day = 86400
...
If you *want* to get the bot phone number , send command *Botphone* .]], 1, "md")
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") then
          if (text:match("^asd$") or text:match("^این دستور نیستش$")) and check_user_channel(msg) then
